...
8. **Every piece with live state crossing a process boundary declares its freshness owner and emits its freshness in the contract.** If the writer and the reader are not the same temporal act — a snapshot, file, or cache that something must regenerate — the piece MUST: (a) have a **named freshness owner** in prod, i.e. *who runs it and at what cadence* (a lifespan thread in `scanner/runtime.py`, never a manual CLI command someone has to remember); and (b) emit its live state through `freshness.LiveSnapshot` so the freshness (`fresco`/`rancio`/`muerto`) lives in the contract — never a mute empty that masks operational death. A new live-state piece with no freshness owner or no `LiveSnapshot` does not merge. The enumerated register is [[docs/superpowers/inventario-estado-vivo.md]]; touching a non-migrated reader from that list without migrating it is a gate violation. Root cause: freshness is a SEMANTIC property of the data, not a cron you pray runs (a screener + a sync that drove F3a shipped through the full pipeline yet were operationally dead in prod — no invoker). See `docs/superpowers/specs/es/2026-06-13-liveness-frescura-huerfanos-design.md`.
