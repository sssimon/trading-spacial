...
- **Double lock: seal + trigger.** `lock_hypothesis` writes the `seal`. The gate, before each fire, **recomputes the seal over the frozen fields and compares** — if a frozen field was edited outside the API (direct UPDATE), the seal mismatches → the bullet is not touched. Plus a SQLite `BEFORE UPDATE` trigger that aborts if a frozen field changes while `status='locked'`. The seal detects; the trigger prevents. Both, because it is irreversible.
- **Threat model — what the seal/trigger do and do NOT cover.** They protect the **frozen claim** fields (`_FROZEN_FIELDS` + `seal`). They deliberately do NOT cover the **lifecycle/control** columns (`status`, `locked_ts`, `fire_authorized_ts`, `fired_ts`, `outcome_ts`, `verdict`, `realized_metric`) — those must stay writable for `authorize_fire`/`record_fire`/`record_outcome`. Consequently, an actor with **direct write access to `signals.db`** could forge a locked/authorized row, flip `status`, or reopen a resolved window. That is **out of scope**: the database file's OS permissions are that boundary, exactly as the AST scanner + `open_holdout` guard the holdout *file* and not the disk. The gate defends against the *code API* being misused (the realistic threat — an operator or a buggy module reaching the bullet without a valid hypothesis), not against an operator with raw SQL deciding to cheat themselves.
- **Clock trust.** `authorize_fire`/`lock_hypothesis` trust the caller-supplied `now`/`today`; there is no independent server clock. The cooldown is deliberation *friction* against the impatient owner, not a cryptographic time-lock — consistent with the single-operator threat model.
...
`lock_hypothesis` refuses unless `deflated_metric` (e.g. `sharpe_deflated`), computed over the **full registry N** via `selection_population_stats()` + `deflation.py`, exceeds the pre-registered `deflated_threshold`. It captures `n_at_lock`. This moves the anti-selection rigor that would otherwise be operator judgment INTO the code: the best-of-N penalty must be paid before the bullet is reachable. The threshold is validated to be a probability in `(0, 1]` (a DSR is a probability; a `≤ 0` threshold would silently neuter the gate, so it is refused).

  **Fail-closed on a degenerate selection population.** The best-of-N penalty `SR*` is driven by `sigma_sr_trials` (the stdev of Sharpes across distinct exploratory configs), which is `None` when fewer than 2 distinct configs have a Sharpe. The `n_effective` floor (50) protects the *count* but NOT the *penalty* — so a near-empty registry would make `SR* = 0` and collapse 4b into "is the Sharpe probably positive?". To prevent that, `lock_hypothesis` **refuses the lock when `sigma_sr_trials is None`** (mirrors the `dsr is None` refusal): you have not earned the shot if the selection population cannot be measured.
