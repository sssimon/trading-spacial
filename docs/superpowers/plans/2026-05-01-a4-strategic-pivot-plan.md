...
| 2026-05-02 | reviewer (post-incident: PR #288 accidentally auto-closed #281) | **Recovery + lesson:** the original PR #288 body included `## Closes` as a section header followed by `Nothing. Plan is meta — coordinates the work tracked in #250, #272, #281, #282, #287...`. GitHub's auto-close parser is syntax-only — it sees `Closes` keyword + proximity to `#281` and triggers auto-close, ignoring negation context like "Closes Nothing". Result: #281 was incorrectly closed at merge time. Recovery: (a) #281 re-opened with explanatory comment, (b) PR #288 body retroactively edited to replace `## Closes` header with safe wording `## Issues referenced (none auto-closed by this PR)`, (c) only #281 was affected (limited blast radius — verified other 4 issues #250/#272/#282/#287 still OPEN). **Process lesson, NEVER repeat:** PR bodies must NOT use `Closes`/`Fixes`/`Resolves` keywords (or `## Closes` headers) when referencing issues that should remain OPEN. For non-closing references, use `References`, `Refs`, `Tracked in`, or `Related to`. The `## Closes` header pattern is especially dangerous because GitHub treats the header as a directive even when the body contains negation. |
